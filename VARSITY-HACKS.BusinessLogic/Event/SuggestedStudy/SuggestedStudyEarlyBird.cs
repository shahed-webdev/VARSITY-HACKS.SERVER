...
public class SuggestedStudyEarlyBird :ISuggestedStudy
{
    ...
    public SuggestedStudyEarlyBird(DateTime startDate, DateTime endDate, DifficultyLevel difficulty, DayOfWeek[] days)
    {
        EventBreak = 30;
        ...
        WeekdayStarTime = TimeSpan.Parse("16:00");
        WeekdayDurationMinute = 540;
        WeekendStarTime= TimeSpan.Parse("9:00");
        WeekendDurationMinute = 480;
    ...
    public void AddSuggestedStudy(int registrationId,int userEventId, DifficultyLevel difficulty, IUnitOfWork db)
    {
        ...
        foreach (DateTime date in EventDates)
        {
            ...
            if (calcCaps.Count > 0)
            {
                ...
                if(maxGap?.Duration.TotalMinutes >= (EventBreak + suggestedStudyDuration))
                {
